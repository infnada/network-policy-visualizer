        ...
        rule.from.forEach((from, i) => {
          ...
          if (from.podSelector) {
            ...
            if (from.podSelector.matchLabels) {
              result += `    MatchLabels:\n`;
              result += Object.entries(from.podSelector.matchLabels)
                .map(([k, v]) => `      ${k}: ${v}`)
                .join('\n') + '\n';
            }
            if (from.podSelector.matchExpressions) {
              result += `    MatchExpressions:\n`;
              result += from.podSelector.matchExpressions.map(expr => 
                `      ${expr.key} ${expr.operator} [${expr.values?.join(', ') || ''}]`
              ).join('\n') + '\n';
            }
          }
          if (from.namespaceSelector) {
            ...
            if (from.namespaceSelector.matchLabels) {
              result += `    MatchLabels:\n`;
              result += Object.entries(from.namespaceSelector.matchLabels)
                .map(([k, v]) => `      ${k}: ${v}`)
                .join('\n') + '\n';
            }
            if (from.namespaceSelector.matchExpressions) {
              result += `    MatchExpressions:\n`;
              result += from.namespaceSelector.matchExpressions.map(expr => 
                `      ${expr.key} ${expr.operator} [${expr.values?.join(', ') || ''}]`
              ).join('\n') + '\n';
            }
          }
        ...
        rule.to.forEach((to, i) => {
          ...
          if (to.podSelector) {
            ...
            if (to.podSelector.matchLabels) {
              result += `    MatchLabels:\n`;
              result += Object.entries(to.podSelector.matchLabels)
                .map(([k, v]) => `      ${k}: ${v}`)
                .join('\n') + '\n';
            }
            if (to.podSelector.matchExpressions) {
              result += `    MatchExpressions:\n`;
              result += to.podSelector.matchExpressions.map(expr => 
                `      ${expr.key} ${expr.operator} [${expr.values?.join(', ') || ''}]`
              ).join('\n') + '\n';
            }
          }
          if (to.namespaceSelector) {
            ...
            if (to.namespaceSelector.matchLabels) {
              result += `    MatchLabels:\n`;
              result += Object.entries(to.namespaceSelector.matchLabels)
                .map(([k, v]) => `      ${k}: ${v}`)
                .join('\n') + '\n';
            }
            if (to.namespaceSelector.matchExpressions) {
              result += `    MatchExpressions:\n`;
              result += to.namespaceSelector.matchExpressions.map(expr => 
                `      ${expr.key} ${expr.operator} [${expr.values?.join(', ') || ''}]`
              ).join('\n') + '\n';
            }
          }
    ...
    if (rule.ports && rule.ports.length > 0) {
      ...
      rule.ports.forEach(port => {
        if (typeof port === 'object') {
          ...
          if (port.endPort !== undefined) portText += `    EndPort: ${port.endPort}\n`;
          if (port.protocol !== undefined) portText += `    Protocol: ${port.protocol}\n`;
          result += portText;
        } else {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl max-h-screen overflow-auto">
        <div className="bg-blue-600 text-white px-4 py-2 flex justify-between items-center">
          <h3 className="text-lg font-semibold">Policy Details: {policy.name}</h3>
          <button 
        ...
        <div className="p-4">
          <div className="mb-4">
            ...
            <div className="bg-gray-100 p-2 rounded font-mono text-sm whitespace-pre-wrap">
              Name: {policy.name}
          ...
          <div className="mb-4">
            <h4 className="font-semibold">Pod Selector</h4>
            <div className="bg-gray-100 p-2 rounded font-mono text-sm whitespace-pre-wrap">
              {policy.podSelector?.matchLabels ? (
                <div>
                  <div>matchLabels:</div>
                  <div className="ml-4">{formatLabels(policy.podSelector.matchLabels)}</div>
                </div>
              ) : 'No matchLabels'}
              ...
              {policy.podSelector?.matchExpressions ? (
                <div>
                  ...
                  <div className="ml-4">{formatExpressions(policy.podSelector.matchExpressions)}</div>
                </div>
              ) : 'No matchExpressions'}
            </div>
          </div>
          ...
          {policy.ingress && policy.ingress.length > 0 && (
            <div className="mb-4">
              <h4 className="font-semibold">Ingress Rules</h4>
              <div className="bg-gray-100 p-2 rounded font-mono text-sm whitespace-pre-wrap">
                {policy.ingress.map((rule, i) => (
                  <div key={`ingress-${i}`} className="mb-2">
                    <div className="font-semibold">Rule {i+1}:</div>
          ...
          {policy.egress && policy.egress.length > 0 && (
            <div className="mb-4">
              <h4 className="font-semibold">Egress Rules</h4>
              <div className="bg-gray-100 p-2 rounded font-mono text-sm whitespace-pre-wrap">
